...
class CroppedImageDataset(Dataset):
    def __init__(self, seq_name_list, frame_id_list, mask_id_list, rgb_path_list, segmentation_path_list):
        '''
    ...
    def __getitem__(self, idx):
        def get_cropped_image(mask, rgb):
            ...
            def draw_boxes_on_image(rgb, mask):
                output_image = rgb.copy()
                for level in range(CROP_SCALES):
                    left, top, right, bottom = mask2box_multi_level(mask, level, 0.1)
                    cv2.rectangle(output_image, (left, top), (right, bottom), (0, 0, 255), 2)  # Red box
                return output_image
            ...
            bboxed_images = draw_boxes_on_image(rgb, mask)
        ...
        bboxed_images = get_cropped_image(mask, np.array(rgb_image))
...
def main():
    ...
    feature_dict = {}
    args.seq_name_list = ['scene0000_00']
    ...
    dataloader = DataLoader(CroppedImageDataset(seq_name_list, frame_id_list, mask_id_list, rgb_path_list, segmentation_path_list), batch_size=64, shuffle=False, num_workers=16)
